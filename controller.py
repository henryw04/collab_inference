...
class Controller:
    def __init__(self):
        ...
        partial_sd = {k: v for k, v in full_sd.items() if any(t in k for t in ["model.embed_tokens", "model.norm", "lm_head"])}
    ...
    def respond(self, obj: State):
        ...
        address = obj.addr
        text = self.tokenizer.decode(obj.input_ids[0], skip_special_tokens=True)
    ...
    def create_log(self):
        ...
        self.logfilename = f"/controller_log/{current_time}.txt"
    ...
    def run(self):
        while True:
            ...
            if self.apiSoc in socks:
                ...
                input_tokens = self.tokenizer(state.prompt, return_tensors="pt").input_ids
            ...
            if self.pullSoc in socks:
                ...
                processors = LogitsProcessorList([
                    RepetitionPenaltyLogitsProcessor(penalty=state.repetition_penalty),
                ...
                if state.remaining_tokens <= 0 or (eos_token_id is not None and next_token.item() == eos_token_id):
                    self.respond(state)
                else:

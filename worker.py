...
class Worker:
    def __init__(self):
        ...
        full_sd = LlamaForCausalLM.from_pretrained(
            ...
            device_map="cpu" ).state_dict()
        target_indices = [f"model.layers.{i}." for i in range(*WorkerConfig.LAYER_RANGES)]+["model.rotary_emb"]
        partial_sd = {
        ...
        self.stage = Stage(model.model.layers[WorkerConfig.LAYER_RANGES[0]:WorkerConfig.LAYER_RANGES[1]])
        self.rotary_emb = model.model.rotary_emb
        self.log(f"Woker{self.worker_id} init success")

    def push(self,obj):
        self.log(f"Push to {"controller" if self.worker_id == 3 else "subsequent worker"}")
        self.pushSoc.send_pyobj(obj)
        self.log(f"Successfully pushed to {"controller" if self.worker_id == 3 else "subsequent worker"}")

    def pull(self):
        self.log(f"Pulling from {"controller" if self.worker_id == 1 else "previous worker"}")
    ...
    def create_log(self):
        ...
        self.logfilename = f"/worker{self.worker_id}_log/{current_time}.txt"
    ...
    def run(self):
        ...
        while True:
            ...
            self.log(f"Successfully received from {"controller" if self.worker_id == 1 else "previous worker"}")
            device = state.hidden_states.device
            seq_len = state.hidden_states.shape[1]
            
            cache_position = torch.arange(0, state.hidden_states.shape[1], device='cpu')

            position_ids = cache_position.unsqueeze(0)

            causal_mask = torch.full(
                (seq_len, seq_len), fill_value=torch.finfo(state.hidden_states.dtype).min, device=device
            )
            causal_mask = torch.triu(causal_mask, diagonal=1)
            causal_mask = causal_mask[None, None, :, :].expand(state.hidden_states.shape[0], 1, seq_len, seq_len)
            position_embeddings = self.rotary_emb(state.hidden_states, position_ids)
            position_ids = torch.arange(0, seq_len, device=device).unsqueeze(0)
            position_embeddings = self.rotary_emb(state.hidden_states, position_ids)

 
            state.set_hidden_states(self.stage(state.hidden_states, attention_mask=causal_mask, position_ids=position_ids, 
